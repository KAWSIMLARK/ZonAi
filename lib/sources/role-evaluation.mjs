...
export function evaluerSubdivisionPossible({
  ...
  // Tailles minimales typiques de lot à TR par catégorie (à valider règlement) :
  //   Habitation desservi (eau+égout) : ~370 m²
  //   Habitation non desservi         : 4 000 m²
  //   Commerce / industrie            : très variable, indicatif 500 m²
  //   Rural                           : 5 000 m²
  let seuilMinimumLot;
  ...
  else if (zonage_categorie === "agricole") seuilMinimumLot = 30000; // pratique CPTAQ
  else seuilMinimumLot = 4000; // ruraux et autres par défaut

  const seuilDouble = seuilMinimumLot * 2;
  if (superficie_m2 < seuilDouble) {
    return {
      statut: "non",
      raison: `Le terrain (${Math.round(superficie_m2)} m²) est trop petit pour deux lots conformes (minimum indicatif ${seuilMinimumLot} m² × 2 pour la catégorie ${zonage_categorie ?? "non précisée"}).`,
      indices: [`Superficie ${Math.round(superficie_m2)} m² < ${seuilDouble} m² (seuil indicatif)`],
    };
  }
