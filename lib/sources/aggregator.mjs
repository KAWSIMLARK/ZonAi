...
export async function lookup(query) {
  ...
  // 3+. Couches géo : inondations + zonage agricole + milieux humides en parallèle
  const [inond, cptaq, humide] = await Promise.all([
  ...
  const parcel = {
    ...
    resume_ia: `Adresse géocodée à ${municipalite}${mrcLabel ? `, ${mrcLabel}` : ""}. ${inondTxt}${humideTxt}${cptaqTxt}`,
