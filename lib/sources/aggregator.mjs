...
export async function lookup(query) {
  ...
  // 3. Couche MELCCFP inondations (point-in-polygon REST ArcGIS)
  const inond = await inondationFor(geo.lat, geo.lon);
  layers.push({
  ...
  // 4. Couche CPTAQ zonage agricole (point-in-polygon local)
  const cptaq = await cptaqFor(geo.lat, geo.lon);
  layers.push({
    name: "CPTAQ_ZONAGE",
  ...
  const contraintes = [...(inond.contraintes ?? []), ...(cptaq.contraintes ?? [])];
  const sources = ["OpenStreetMap / Nominatim"];
