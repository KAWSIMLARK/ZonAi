// Règles métier simulées pour ZonAI.
...
export function validateParcel(parcel) {
  ...
  if (parcel.region && parcel.region !== "Mauricie") {
    issues.push({
      code: "OUT_OF_REGION",
      field: "region",
      message: `La région « ${parcel.region} » est hors couverture ZonAI.`,
    });
  }
...
export function deriveWarnings(parcel) {
  ...
  if (parcel.region && parcel.region !== "Mauricie") {
    out.push({
      ...
      message: `Cette fiche se situe en région « ${parcel.region} », hors du périmètre couvert par ZonAI.`,
    });
  }
